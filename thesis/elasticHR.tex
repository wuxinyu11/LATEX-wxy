...
\end{equation}
式中$C_{ijkl}^{-1}$为四阶弹性张量\ref{}的逆。从式中可知，本质边界条件已存在能量泛函中。而外力边界条件则通过拉格朗日乘子法进行施加，包括体力项$\pmb b$和外力项$\pmb t$，此时Hellinger-Reissner的能量泛函为：
\begin{equation}
\label{Ehellinger}
...
首先，位移分量$u_i$通过无网格形函数\ref{}进行近似，近似的位移$u^h_i$可表示为：
...
内嵌局部积分约束条件$\tilde{g}_{iI}$，进而满足全域积分约束条件，从而保证算法的计算精度和误差收敛性。这意味着通过构造再生光滑梯度，可以确保在进行全域积分时满足积分约束条件，从而得到准确的结果，并保证算法的计算精度和误差收敛性。反之，Hellinger-Reissner变分原理的混合离散框架也完备了再生光滑梯度法的变分理论基础。\par
...
\begin{equation}\label{1}
    % \sum_{C=1}^{N\!C}\sum_{I=1}^{N\!P}\delta d_{iI}(\tilde{g}_{jI}^T-\bar{g}_{jI}^T)\pmb{a}_{ij}=\sum_{I=1}^{N\!P}\delta\pmb{d}_I^T\pmb{f}_I
    \sum_{C=1}^{N\!C}\sum_{I=1}^{N\!P}\delta d_{iI}(\tilde{g}_{jI}^T-\bar{g}_{jI}^T)\pmb{a}_{ij}
\end{equation}
% 其中力向量$\pmb{f}$的具体表达式为：
% \begin{equation}\label{CH4-f}
...
\end{split}
\end{equation}
% 其中$\tilde{\pmb{B}}_I$是由再生光滑梯度$\tilde{\Psi}_{I,i}$组成的梯度矩阵，其具体表达式为：
% \begin{equation}
...
\end{split}
\end{equation}
% 其中$\bar{\pmb{B}}_I$是由再生光滑梯度$\bar{\Psi}_I$组成的梯度矩阵，其具体表达式为：
% \begin{equation}
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}\par
% 此时，将式(\ref{CH4-1})的右侧改写为如下形式：
% \begin{equation}
%     \sum_{I=1}^{N\!P}\delta\pmb{d}^T\pmb{f}
% \end{equation}
% 其中力向量$\pmb{f}_I$的具体表达式为：
% \begin{equation}\label{CH4-f}
%     \pmb{f}_I=\int_{\Gamma^t}\Psi_I\pmb{t}d\Gamma+\int_{\Omega}\Psi_I\pmb{b}d\Omega
% \end{equation}
% 进一步引入力向量$\pmb{f}_I$表达式(\ref{CH4-f})和式(\ref{simplify})代入到式(\ref{CH4-1})中得到
% Hellinger-Reissner变分原理下的弹性力学问题离散控制方程式：
% \begin{equation}\label{equationE}
% (\pmb{K}+\pmb{\tilde{K}}+\pmb{\bar{K}})\pmb{d}=\pmb{f}+\tilde{\pmb{f}}+\bar{\pmb{f}}
% \end{equation}\par
