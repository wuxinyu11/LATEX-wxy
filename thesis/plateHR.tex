...
\end{split}
\end{equation}
式中的最后一个等式通过引入式(\ref{PG})-(\ref{PBPSI})和(\ref{MVP})进行化简得到，具体详细的推导可参考附录B薄板HR变分原理的本质边界条件施加方法推导过程。根据式(\ref{CH5-2})和(\ref{Pzuzhuang})代入到式(\ref{CH5-1})中可得到赫林格-赖斯纳变分原理下的薄板问题离散控制方程：
...
同样，基于赫林格-赖斯纳变分原理的特点，薄板的离散控制方程式(\ref{PHR3})中的稳定项$\bar{K}_{IJ}$、$\bar{f}_I$中已经内嵌，相较于Nistche法(\ref{PKFs})中的稳定项$K^s_{IJ}$、$f^s_I$而言不需要引入罚函数项，不会因为人工参数的存在继而影响计算精度。
同样，根据赫林格-赖斯纳变分原理在求解薄板问题过程中也用到了优化的数值积分方案见附录C无网格法优化的数值积分方案。
\section{数值算例}
\subsection{分片实验}
...
\begin{figure}[H]
    ...
    \end{subcaptiongroup}
\caption{\centering{简支方板问题三次基函数误差对比：\\\subref{RCL2} $L_2$误差；\subref{RCH1} $H_1$误差；\subref{RCH2}$H_2$误差；\subref{RCH3} $H_3$误差}}
\label{RCLH}
\end{figure}
\newpage
\begin{figure}[H]
    ...
    \end{subcaptiongroup}
\caption{\centering{简支方板问题四次基函数误差对比：\\\subref{RQL2} $L_2$误差；\subref{RQH1} $H_1$误差；\subref{RQH2}$H_2$误差；\subref{RQH3} $H_3$误差}}
\label{RQLH}
\end{figure}
\begin{figure}[H]
    \centering
    ...
    \end{subcaptiongroup}
\caption{\centering{简支方板问题三次基函数效率对比：\\\subref{Ccputime}计算时间与节点数的关系；\subref{Cefficiency}本质边界条件施加效率分析}}
\label{RCcputime}
\end{figure}
\newpage
\begin{figure}[H]
    ...
    \end{subcaptiongroup}
\caption{\centering{简支方板问题四次基函数效率对比：\\\subref{Qcputime}计算时间与节点数的关系；\subref{Cefficiency}本质边界条件施加效率分析}}
\label{RQcputime}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subcaptiongroup}
\caption{\centering{简支等边三角形板问题三次基函数误差对比：\\\subref{TCL2} $L_2$误差；\subref{TCH1} $H_1$误差；\subref{TCH2}$H_2$误差；\subref{TCH3} $H_3$误差}}
\label{TCLH}
\end{figure}
\newpage
\begin{figure}[H]
    ...
    \end{subcaptiongroup}
\caption{\centering{简支等边三角形板问题四次基函数误差对比：\\\subref{TQL2} $L_2$误差；\subref{TQH1} $H_1$误差；\subref{TQH2}$H_2$误差；\subref{TQH3} $H_3$误差}}
\label{TQLH}
\end{figure}
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \end{subcaptiongroup}
\caption{\centering{简支环行板问题四次基函数误差对比：\\\subref{AQL2} $L_2$误差；\subref{AQH1} $H_1$误差；\subref{AQH2}$H_2$误差；\subref{AQH3} $H_3$误差}}
\label{AQLH}
\end{figure}
\newpage
\begin{figure}[H]
    ...
    \end{subcaptiongroup}
\caption{\centering{简支环形板问题四次基函数效率对比：\\\subref{aQcputime}计算时间与节点数的关系；\subref{aQefficiency}本质边界条件施加效率分析}}
\label{AQcputime}
\end{figure}
\begin{figure}[H]
    \centering
