...
\end{cases}
\end{equation}\par
对式(\ref{Pnitsche})进行变分得到薄板问题施加本质边界条件Nitsche法的等效积分弱形式为：
...
\end{subequations}
\par
值得注意的是，Nitsche法作为目前伽辽金无网格法常采用的满足变分一致性的本质边界条件施加方法，但Nitsche法的稳定项中包含罚因子，选择不当的罚因子可能会导致数值不稳定性，
