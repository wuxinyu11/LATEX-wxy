...
针对HR伽辽金弱形式中弯矩的近似\cite{wu2022HR}，首先将薄板中面$\Omega$划分为一系列背景积分单元$\Omega_C$，$C=1,2,\dots,n_c$，并且有$\cup_{C=1}^{n_c}\Omega_C=\Omega$。在背景积分单元$\Omega_C$内，弯矩分量$M_{\alpha\beta}$的近似表达式$M^h_{\alpha\beta}$可表示为：
\begin{equation}\label{monent}
...
\end{equation}
$\pmb a_{\alpha\beta}$是积分域$\Omega_C$的常系数向量，将近似的弯矩式(\ref{monent})代入
...
\end{split}
\end{equation}
其中，$\Gamma_C$是$\Omega_C$的边界。将式(\ref{ca})代入到式子(17)中可以得到$\pmb{M}^h_{\alpha\beta}$的表达式：
\begin{equation}
\begin{split}
